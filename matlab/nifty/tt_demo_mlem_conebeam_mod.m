...
%mask = et_spherical_phantom(Nx,Ny,Nz,min([Nx Ny Nz])/2-2,1,0,(Nx+1)/2,(Ny+1)/2,(Nz+1)/2);
mask = 1;
...
volume_size_mm               = [Nx,Ny,Nz];        % mm
detector_size_mm_0           = 1.2*[180,480];      % mm 
...
p = tt_project_ray_mex(attenuation,volume_size_mm,source_location_mm,int32(detector_size_pix),detector_size_mm,detector_translation_mm,detector_rotation_rad,ray_step_vox,GPU);
p = exp(-p); 
min_invp = min(1./p(:)); 
max_invp = max(1./p(:)); 
