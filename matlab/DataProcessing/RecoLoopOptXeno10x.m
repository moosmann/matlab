function RecoLoopOptXeno10x(alpha,evalTIElo,evalTIEpnlo,evalCTF,BinaryFilter,DataSetsToProcess,EnergyDistancePixelsize,ParentPath,DataFolderNamePrefix)
...
if nargin < 4
    evalCTF      = 1;%Evaluate contrast-transfer-function (CTF) expression.
end
if nargin < 5
...
if nargin < 7
    ...
    offset = 0.023;
    EnergyDistancePixelsize{1} = [22 offset+0.100 1.5e-6];
    EnergyDistancePixelsize{2} = [22 offset+0.100 1.5e-6];
    EnergyDistancePixelsize{3} = [22 offset+0.400 1.5e-6];
    EnergyDistancePixelsize{4} = [22 offset+0.400 1.5e-6];
end
if nargin < 8
