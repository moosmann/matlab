function p07_reco_loop_hzb_for_battery_11013084( SUBSETS, RUN_RECO, PRINT_PARAMETERS)

%
% Created on 06-May-2022 by moosmanj
...
image_correlation.area_width = [1 50];%[-100 1];% correlation area: index vector or relative/absolute position of [first pix, last pix], negative indexing is supported
image_correlation.area_height = [0.7 1]; % correlation area: index vector or relative/absolute position of [first pix, last pix]
...
raw_path = '/asap3/petra3/gpfs/p07/2022/data/11013084/raw/';
par.raw_bin = 3;
par.raw_roi = [991 1650];

tomo.rot_axis_offset = 5.000000 / 3 * par.raw_bin;
write.to_scratch = 1;
write.flatcor = 0;
par.scan_path = [raw_path 'for003_ag97_127_4a_step0'];

ring_filter.apply = 1;
ring_filter.apply_before_stitching = 1; % ! Consider when phase retrieval is applied !
ring_filter.method = 'wavelet-fft';

% wavelet type, see 'FilterStripesCombinedWaveletFFT' or 'waveinfo'
wname = {'db7';'db25';'db30'};
wsigma = [1 3 5 7 9];
wdl = {1:3,1:5,1:7,1:9,1:11,1:13, 3:5,3:7,3:9,3:11,3:13, 5:7,5:9,5:11,5:13, 7:9,7:11,7:13, 9:11,9:13};
for m = 1:numel(wdl)
    for k = 1:numel(wname)
        for l = 1:numel(wsigma)
            
            ring_filter.waveletfft_wname = wname{k};
            ring_filter.waveletfft_sigma = wsigma(l); %  s
            ring_filter.waveletfft_dec_levels = wdl{m};
            
            write.parfolder = sprintf( 'ringfilt_waveletfft_%s_sigma%02u_declevels%s', ring_filter.waveletfft_wname, ring_filter.waveletfft_sigma, sprintf('_%u', ring_filter.waveletfft_dec_levels));
            %fprintf('\n %s', write.parfolder)
            ADD
        end
    end
