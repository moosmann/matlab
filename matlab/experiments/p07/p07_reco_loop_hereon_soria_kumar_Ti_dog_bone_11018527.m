function p07_reco_loop_hereon_soria_kumar_Ti_dog_bone_11018527( SUBSETS, RUN_RECO, PRINT_PARAMETERS)
...
%
% Created on 05-May-2024 by moosmanj
...
par.gpu_index = [1:10]; % integer vector: indices of GPU devices to use, Matlab notation: index starts from 1. default: [], uses all
...
par.poolsize_gpu_limit_factor = 0.4; % scalar: elative amount of GPU memory used for preprocessing during parloop. High values speed up Proprocessing, but increases out-of-memory failure
...
%% PARAMETER / DATA SETS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
proc_path = '/asap3/petra3/gpfs/p07/2024/data/11018527/processed/';
raw_path = '/asap3/petra3/gpfs/p07/2024/data/11018527/raw/';

par.scan_path = [proc_path 'fk001_p3_a']; ADD
par.scan_path = [proc_path 'fk001_p3_b']; ADD

par.scan_path = [proc_path 'fk002_p2_a']; ADD
par.scan_path = [proc_path 'fk003_p1_a']; ADD
par.scan_path = [proc_path 'fk004_pl_125_6b_a']; ADD
par.scan_path = [proc_path 'fk005_pl_125_2b_a']; ADD
par.scan_path = [proc_path 'fk006_pl_63_6b_a']; ADD
par.scan_path = [proc_path 'fk007_pl_63_4b_a']; ADD
...
pixel_filter_sino.threshold_hot = 0.001;
pixel_filter_sino.threshold_dark = 0.0001;
...
par.poolsize_gpu_limit_factor = 0.4;
pixel_filter_sino.use_gpu = par.use_gpu_in_parfor;
ring_filter.apply = 0;
...
par.scan_path = [proc_path 'rk_001_s1'];
par.nexus_path = [raw_path 'rk001_s1_a'];
tomo.rot_axis_offset = 0.3;
ADD

par.scan_path = [proc_path 'rk_002_s2']; 
par.nexus_path = [raw_path 'rk002_s2_a'];
tomo.rot_axis_offset = 0.65;
ADD

par.scan_path = [proc_path 'rk_003_s3']; 
par.nexus_path = [raw_path 'rk003_s3_a'];
tomo.rot_axis_offset = -0.25;
ADD

par.scan_path = [proc_path 'rk_004_s4']; 
par.nexus_path = [raw_path 'rk004_s4a'];
tomo.rot_axis_offset = 0.65;
ADD

interactive_mode.rot_axis_pos = 1;
par.scan_path = [proc_path 'rk_005_s5'];
par.nexus_path = [raw_path 'rk005_s5a'];
tomo.rot_axis_offset = 0.15;
ADD

par.scan_path = [proc_path 'rk_006_s6'];
par.nexus_path = [raw_path 'rk006_s6a'];
tomo.rot_axis_offset = 0.0;
ADD

par.scan_path = [proc_path 'rk_007_s7'];
par.nexus_path = [raw_path 'rk007_s7a'];
tomo.rot_axis_offset = 0.15;
ADD

par.scan_path = [proc_path 'rk_008_s8'];
par.nexus_path = [raw_path 'rk_008_s8a'];
tomo.rot_axis_offset = 0.0;
ADD

par.scan_path = [proc_path 'rk_009_s9']; 
par.nexus_path = [raw_path 'rk_009_s9a'];
ADD

par.scan_path = [proc_path 'rk_010_s10'];
par.nexus_path = [raw_path 'rk_010_s10a'];
ADD

tomo.rot_axis_search_auto = 0; % find extrema of metric within search range
tomo.rot_axis_search_range = 0.0 + (-2:0.1:2); % search reach for automatic determination of the rotation axis offset, overwrite interactive result if not empty
tomo.rot_axis_search_metric = 'abs'; % string: 'neg','entropy','iso-grad','laplacian','entropy-ML','abs'. Metric to find rotation axis offset
tomo.rot_axis_search_extrema = 'max'; % string: 'min'/'max'. chose min or maximum position
