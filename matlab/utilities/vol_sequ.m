function [vol, vol_reg] = vol_sequ( p )
% Create images sequences from 4D load tomography data.
%
% Script to process in 4D image squences  Read in
% tomo data sets, convert to 8bit scaled by given thresholds, and save 
...
%% DEFAULT ARGUMENTS %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
proc_path = assign_from_struct( p , 'proc_path', '/asap3/petra3/gpfs/p07/2019/data/11006991/processed' );
scan_pattern = assign_from_struct( p, 'scan_pattern', 'syn0*135*' );
save_name = assign_from_struct( p, 'save_name', 'syn_zSequence_Ti_12w_47R_t135ms' );
reco_sub = assign_from_struct( p , 'reco_sub', 'reco/float_rawBin4' );
...
register = assign_from_struct( p , 'register', 1 );
...
voxel_size = assign_from_struct( p , 'voxel_size', 3.77 );
...
scan_path = [proc_path filesep scan_pattern];
save_path = [proc_path filesep save_name];
...
p = [proc_path filesep struct_scans(1).name filesep reco_sub];
...
for nn = 1:numel( steps )
    fprintf( '\n %2u : %s.', nn, struct_scans(nn).name)
        ...
        % Read
    p = [proc_path filesep struct_scans(nn).name filesep reco_sub];
    struct_slices = dir( [p filesep '*.tif']);
    num_slices = numel( struct_slices );
    ...
    figure( 'Name', sprintf( 'Check thresholding for 8bit conversion: vol # %u', nn) )
    subplot( 1, 2, 1 )
    imsc(im);
...
%% Slice: Animated gif
gif_path = [ save_path filesep 'gif' ];
...
filename = sprintf( '%s/%s_loadSequ_x%04u_%s.gif', gif_path, save_name, xx, barstr );
...
fprintf( '\n scalebar string : %s', barstr )
filename = sprintf( '%s/%s_loadSequ_y%04u_%s.gif', gif_path, save_name, yy, barstr );
fprintf( '\n output file: %s', filename)
for nn = 1:size(vol_reg,4)
...
fprintf( '\n scalebar string : %s', barstr )
filename = sprintf( '%s/%s_loadSequ_z%04u_%s.gif', gif_path, save_name, zz, barstr );
fprintf( '\n output file: %s', filename)
for nn = 1:size(vol_reg,4)
...
for mm = 1:size( vol_reg, 4 )
    filename = sprintf( '%s/%s_loadSequ_%02u.tif', tif_path, save_name, mm );
...
if ~isempty( crop_roi )
    for mm = 1:size( vol_reg, 4 )
        filename = sprintf( '%s/%s_loadSequ_%02u.tif', scan_path, save_name, mm );
        for nn = 1:size( vol_reg, 3 )
            im = squeeze( vol_reg(:,:,nn,mm) );
            if nn == 1
                imwrite( im, filename, 'Compression','none');
