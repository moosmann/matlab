function FlatCorLCI2(ParentPath,DataSet,TomoSetsToProcess,Cropping,GaussianWidth,HotPixThres_DarkFlatData,doDarkFieldCorrection,NumOfDarks,NumOfFlats,DarksBeforeFlats)
...
for ff = 1:numel(GaussianWidth)
    IntNamePostfix = sprintf('filtBandpassGW%03u%s%s_filtHotPixelD%03uF%03uS%03u',GaussianWidth(ff),cropmeth,dodark,100*HotPixThres_DarkFlatData);
    ...
    if (newYdim>1) || (newXdim>1)
        ...
        fprintf('\nCROPPING IMAGES from [ver hor] = [%u %u] to [%u %u]!',dim1,dim2,newXdim,newYdim)
    else
        dimx = dim1;
        dimy = dim2;
    end
    ...
    w1x = 1- window(@gausswin,dimx,GaussianWidth(ff));
    w1y = 1- window(@gausswin,dimy,GaussianWidth(ff));
    [wx wy] = meshgrid(w1y,w1x);
    w = fftshift(wx.*wy);
    w(1,1) = 1;% Restore mean
    edfwrite(sprintf('%sFourierspaceFilterProfileX_%s.edf',OutputPathPre,IntNamePostfix),w1x,'float32');
    edfwrite(sprintf('%sFourierspaceFilterProfileY_%s.edf',OutputPathPre,IntNamePostfix),w1y,'float32');
    %% Loop over tomograms
    if TomoSetsToProcess == 0
