...
% DataProc32IDstack201307tomoWithStatistics
%
...
if ~exist('DataSet','var')
    ...
    DataSet = 'Xenopus_atlas/stage02cell';
...
for tt = 1:NumTomos
    ...
    if doFBP || savePhaseMaps
        ...
        % No phase retrieval nor FBP. Only reading, pixel filtering,
        % masking, flat-/darkfield correction, and saving
        parfor nn = 1:dim3
            filename = sprintf('%s%s',InputPath,projNames{nn+projOffset});
            im = FilterPixel(single(imread(filename,'tif','PixelRegion',PixelRegion)),hptProj);
            im = (im - darkMean)./flatMean;
    ...
    if doFBP || savePhaseMaps        
        ...
        % Save sino filtered intensities
        if ~isempty(saveIntMaps)
            % Modify write path for filtered int maps
            IntOutputPath = [IntOutputPath(1:end-1) '_sinoFilt/'];
            CheckTrailingSlash(IntOutputPath)
            CheckAndMakePath(IntOutputPath);
            % .par file creation
            ParInputFilePrefix = sprintf('%sint_',IntOutputPath);
            ...
            CheckTrailingSlash(IntVolOutputPath);
            CheckAndMakePath(IntVolOutputPath);
            ParOutputFilePrefix = sprintf('%ssinoFilt_tomo%02u.vol',IntVolOutputPath,tomoInd);
            MakeParFile32ID(ParInputFilePrefix,ParOutputFilePrefix,[dim1 newdim2],RotAxisPosPadded,'NumberOfProjections',dim3,'OverallAngle',180*(1-projOffset/NumProj));
        end
        %% Phase retrieval
        ...
        fprintf('\n Sino filtering and phase retrieval finished. Elapsed time: %.2g min',toc/60);
    end
    %% Save phase maps
%     if savePhaseMaps
%         switch parFlag
...
%     fprintf('\n Use ''for''-loop over 3rd index n: \n'); 
%     for nn = 1:dim3
%         fprintf('%4u ',nn);if mod(nn,printMod)==0,fprintf('  \n   ');end
%         stack(:,:,nn) = fft(stack(:,:,nn),[],2);
%     end
% else
%     stack = fft(stack,[],2);
% end
...
% if stackSize < maxSizeArrayGB/1.8;
%     stack = stack.*repmat(phaseFilter,[1 1 dim3]);
% elseif stackSize < maxSizeArrayGB/1.3
...
% else
%     fprintf('\n Use ''for''-loop to apply phase filter due to stack size.')
%     for nn = 1:dim3
%         fprintf('%4u ',nn);if mod(nn,printMod)==0, fprintf('  \n   ');end
%         stack(:,:,nn) = phaseFilter.*stack(:,:,nn);
%     end
% end
