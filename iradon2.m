...
x     = -floor((N-1)/2):floor(N/2);
[x,y] = meshgrid(x,-x);

% index of the center of the projections
...
switch interp
    ...
    case {'spline','pchip','cubic','v5cubic'}
        ...
        img = zeros(N,class(p));        
        % Input arguments for interpolation 
        % backprojection
        %Consider a fixed coordinate system (x,y) containing the sample to
        %be reconstructed. Given are the projections p in coordinate system
        %(x',y') which is rotated about an angle theta w.r.t. the unprimed
        %system (x,y). Thus, one needs to know the value of the projection on
        %each grid point (x,y), which is p = p(x') =
        %p(x*cos(theta)+y*sin(theta)
        taxis = (1:size(p,1)) - ctrIdx;
        taxis = taxis(:);
        img = img(:);
        parfor nn=1:length(theta)    
            proj = p(:,nn);
            proj = proj(:);
            xprime = x.*costheta(nn) + y.*sintheta(nn);
            xprime = xprime(:);
            % GPU
%             img = gpuArray(img);
%             taxis = gpuArray(taxis);
%             xprime = gpuArray(xprime);            
%             proj = gpuArray(proj);
            % Interpolation
            img = img + interp1(taxis,proj,xprime);
        end
        % Reshape line vector img
        img = reshape(img,N,N);
end
img = gather(img);
...
for i=3:nargin
    ...
    if ischar(arg)
        ...
        idx = find(strcmpi(arg,string_args));
        if isempty(idx)
            error(message('images:iradon:unknownInputString', arg))
        elseif numel(idx) > 1
            error(message('images:iradon:ambiguousInputString', arg))
        elseif numel(idx) == 1
