function [darkMean flatMean] = FlatCor32ID(TomoSetsToProcess,useGPU,showFigures,varargin)
% Data preprocessing for the experiment 'Comparative Life Cell Imaging' at
% 32-ID-C at APS in October 2012

tic; tpre = 0; ttotal = 0; tpreloop = 0;
%% Default arguments
if nargin < 1
    TomoSetsToProcess = {};%:13
end
if nargin < 2
    useGPU = 1;
end
...
for tt = 1:NumTomos
    ...
    parfor nn = 1:NumPreFlat-offset
        ...
        %[stack(:,:,nn) hpfPreFlat(nn)] = FilterHotPixel(single(imread(filename,'tif')),hptFlat);
        %stack(:,:,nn) = FilterHotPixel(single(imread(filename,'tif'))-darkMean,hptFlat);
    end
    %% Loop preFlats: Fourier space filtering
    if FSfiltering(2)
        parfor nn = 1:dim1
            im = squeeze(stack(nn,:,:));
            im = fft2(im);
...
%     end
    ...
    im2 = fliplr(im2);
    % Crop vertically
    x = floor(2*dim1/10):ceil(dim1*9/10);
    %x = 1:dim1;
    im  = im(x,:);
    im2 = im2(x,:);
    ...
    %% Loop Projections: Phase retrieval
