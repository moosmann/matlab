...
= RecoSchwingCTF(dataslice,sn_smax,lambda,distance,pixelsize,padding,padvalue,compute_correction);
...
eta        = fftshift(eta);
xieta      = xi(:,:,1).*eta(:,:,1);
selap      = sign(sin(1/prefactor*(xi.^2 + eta.^2)/2)).*exp(-s.*(abs(sin(1/prefactor*(xi.^2 + eta.^2)/2))));
...
phi1 = 1/2*sum(ifft2(selap.*repmat(fft2(g),[1,1,sn])).*w,3);
...
phi2 = 1/2*sum(ifft2(selap.*repmat(fft2(lap_phi2),[1,1,sn])).*w,3);
