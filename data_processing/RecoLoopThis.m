function [sinotie sinopctf] = RecoLoopThis(alphaCTF_alphaTIE,evalTIElo,evalTIEpnlo,evalCTF,BinaryFilterThreshold,EnergyPixelsizeDistance,InputPath,FileNamePrefix,OutputPath,doSino,IndexArray,DataSetName)
...
DataFormat = DataFormatStruct(10).name(end-2:end);
...
if doSino
    if evalTIElo
        sinotie = zeros(dim1,NumOfProj-5);
    end
    if BinaryFilterThreshold(1) > 0
        sinopctf = zeros(dim1,NumOfProj-5);
    end
end
%% Start looping.
fprintf('Projection which is currently processed and saved:\n')
for nn = 1:1:NumOfProj
    ...
    if evalTIElo
        ...
        if doSino && (nn <= NumOfProj -5)
            sinotie(:,nn) = tieLO(:,floor(SinoSliceNum));
        end
    end
    if evalTIElo
